...
const jenkinsUpdateJob =
  "http://hfp.northeurope.cloudapp.azure.com:8080/job/NotifyAdmin/buildWithParameters?";
...
const determineTypeOfFinanceKey = (key) => {
  if (key !== "all") {
    if (key.substring(key.length - 3, key.length) === "000") {
      ...
      key.substring(key.length - 2, key.length) === "00" ||
      key.substring(1, 3) == "00"
    ) {
      return "Secondary";
    } else {
      return "";
...
api.post("/updateDatabase", (req, res) => {
  ...
  const requestURL =
    jenkinsUpdateJob + "userEmail=" + email + "&year=" + year + "&token=fetch";
...
*/
api.get(
  "/expenses/:per/:lvl/:agroup/:aff/:dgroup/:dep/:fin/:cre",
  (req, res) => {
        ...
        (doc) => {
          ...
          for (let i = 0; i < labels.length; i++) {
            if (labels[i].label === "Creditor") {
              ...
              labels[i].key = doc.hits.hits[0]._source[
                labels[i].label
              ].substring(0, labels[i].key.length);
              labels[i].label = doc.hits.hits[0]._source[
                labels[i].label
              ].substring(labels[i].key.length + 1);
            }
          }
            ...
            .then(
              (docum) => {
                ...
                const totalDebit = Math.abs(
                  docum.aggregations.total_amount.value
                );
                elasticClient
                  .search({
                    index: ind,
...
api.get("/joint-revenue/:per/:lvl/:dep/:fin/:cre", (req, res) => {
  ...
  if (financeKeyID !== "all") {
    if (financeKeyID.substring(1, 4) === "000") {
      ...
      financeKeyID.substring(2, 4) === "00" ||
      financeKeyID.substring(1, 3) === "00"
    ) {
      mustFinanceKey = {
      ...
      (doc) => {
        ...
        for (let i = 0; i < labels.length; i++) {
          if (labels[i].label === "Creditor") {
            ...
            labels[i].key = doc.hits.hits[0]._source[labels[i].label].substring(
              0,
              labels[i].key.length
            );
            labels[i].label = doc.hits.hits[0]._source[
          ...
          .then(
            (docum) => {
              ...
              elasticClient
                ...
                .then(
                  (docume) => {
                    const totalYear = Math.abs(
                      docume.aggregations.total_year.value
                    );
                    const respObj = {
                      slices,
                      totalCredit,
 ...
 */
api.get(
  "/special-revenue/:per/:lvl/:agroup/:aff/:dgroup/:dep/:fin/:cre",
  (req, res) => {
    let period = req.params.per;
    const level = req.params.lvl;
    const affairGroupID = req.params.agroup;
